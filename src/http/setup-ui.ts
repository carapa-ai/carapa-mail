 ...
 */
export function getSetupPage(options: { allowSignup: boolean; hasToken: boolean; publicHostname: string; smtpPort: number; smtpInsecurePort: number; imapProxyPort: number; mcpPort: number; mcpEnabled: boolean; mcpPublicUrl: string; allowPromptOverride: boolean; allowPromptAppend: boolean } = { allowSignup: false, hasToken: false, publicHostname: '', smtpPort: 2525, smtpInsecurePort: 0, imapProxyPort: 1993, mcpPort: 3466, mcpEnabled: false, mcpPublicUrl: '', allowPromptOverride: true, allowPromptAppend: true }): string {
  ...
  html = html.replace(/\{\{\s*SMTP_INSECURE_PORT\s*\}\}/g, String(options.smtpInsecurePort));
