...
export function readSecret(envName: string, secretName?: string): string {
  const fileName = secretName || envName.toLowerCase();
  try {
    const val = fs.readFileSync(path.join(SECRETS_DIR, fileName), 'utf8').trim();
    if (val) return val;
  } catch { /* ignore */ }
...
export const SMTP_INSECURE_PORT = parseInt(process.env.SMTP_INSECURE_PORT || '0', 10);

// AI filtering
export const ANTHROPIC_AUTH_TOKEN = readSecret('ANTHROPIC_AUTH_TOKEN', 'anthropic_api_key');
...
// Admin
export const CARAPA_MAIL_TOKEN = readSecret('CARAPA_MAIL_TOKEN', 'carapa_mail_token');
...
export const CARAPA_MAIL_ENCRYPTION_KEY = readSecret('CARAPA_MAIL_ENCRYPTION_KEY');
